...
import { MapPin, Phone, Mail, Clock, ArrowRight, AlertCircle, Map, Navigation, Star, ExternalLink } from 'lucide-react';
    ...
    <div className={`${sectionClass} max-w-7xl mx-auto px-6`}>
      ...
      >
        {slice.items.map((item, idx) => {
          ...
          const bookingText = item.booking_text || '';
          const bookingUrl  = item.booking_url || '';
                  ...
                  <div className="space-y-4 my-6 text-sm text-[#6A5B5E]">
                    ...
                    {bookingUrl && (
                      <a 
                        ...
                        className="flex items-center gap-2 font-bold text-[#511B29] hover:text-[#5d1726]/80 transition-colors w-fit"
                      >
                        <ExternalLink className="w-4 h-4 text-[#de7c8a] shrink-0" />
                        <span>{bookingText || (bookingUrl.includes('piearsta.lv') ? 'piearsta.lv' : (isEn ? 'Book an appointment' : 'Pieteikt vizīti'))}</span>
                      </a>
                    )}
