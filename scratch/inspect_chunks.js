...
${clinic.accessibility_alert ? `Piezīme par pieejamību: ${clinic.accessibility_alert}` : ''}`;
    ...
    const issues = chunks.filter(c => !c.text || c.text.trim().length < 10 || c.text.length > 1200);
    console.log(`Chunks with potential issues (length > 1200 or < 10): ${issues.length}`);
    issues.forEach((iss, i) => {
      console.log(`\nIssue #${i+1}: Length = ${iss.text.length}, Source = ${iss.source}`);
      console.log(iss.text.substring(0, 300) + '...');
    });
  })
  .catch(console.error);
